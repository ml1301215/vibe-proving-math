...
from core.theorem_search import search_theorems as raw_search
...
async def _safe_verify(proof: str, statement: str) -> Optional["VerificationResult"]:
    """安全调用 verify_sequential，捕获异常返回 None。"""
    try:
        return await verify_sequential(proof, statement)
    except Exception as e:
        logger.warning("verify_sequential failed: %s: %s", type(e).__name__, e)
        return None
...
async def _attempt_proof_with_revision(
    ...
    for rev in range(max_revisions):
        ...
        if verify is None or not verify.has_errors():
...
async def _solve_inner(statement: str, model=None, progress: ProgressCb = None, lang: Optional[str] = None, extra_context: str = "") -> SolverResult:
    ...
    try:
        direct_hits = await search_theorems(statement, top_k=5, min_sim=0.0)
        if direct_hits and direct_hits[0].similarity >= 0.75:
            top = direct_hits[0]
            return SolverResult(
    ...
    if proof.is_successful() and (verify_result is None or not verify_result.has_errors()):
        ...
        return SolverResult(
            ...
            confidence=proof.confidence if verify_result else proof.confidence * 0.85,
        ...
        if ce_result.found and ce_result.confidence >= 0.75:
            ...
            return SolverResult(
                blueprint=(
                    ...
                    + f"> 置信度 {ce_result.confidence:.0%}。建议重新检查命题条件。"
                ),
                references=[],
